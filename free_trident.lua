...
if not LPH_OBFUSCATED then
    ...
    SWG_DiscordUser = "liver"
    SWG_DiscordID = 1337
    SWG_Private = true
    SWG_Dev = false
    SWG_Version = "free"
    SWG_Title = 'paid liverCheat.xyz %s - %s | soso trident'
    SWG_ShortName = 'paid'
    SWG_FullName = 'ts'
    SWG_FFA = false
end;
...
local function getfile(name)
    local repo = "https://raw.githubusercontent.com/annatelechuk93-hub/liver/refs/heads/main/"
...
local function isliverfile(file)
    return isfile("liver/new/files/"..file)
end
local function readliverfile(file)
    if not isliverfile(file) then return false end
    local success, returns = pcall(readfile, "liver/new/files/"..file)
    if success then return returns else return print(returns) end
end
local function loadliverfile(file)
    if not isliverfile(file) then return false end
    local success, returns = pcall(loadstring, readliverfile(file))
    if success then return returns else return print(returns) end
end
local function getliverasset(file)
    if isliverfile(file) then return false end
    local success, returns = pcall(getcustomasset, "liver/new/files/"..file)
    if success then return returns else return print(returns) end
end
do
    if not isfolder("liver") then makefolder("liver") end
    if not isfolder("liver/new") then makefolder("liver/new") end
    if not isfolder("liver/new/files") then makefolder("liver/new/files") end
    local function getfiles(force, list)
        for _, file in list do
            if (force or not force and not isliverfile(file)) then
                writefile("liver/new/files/"..file, getfile(file))
            end
        end
    end
    local gotassets = getfile("assets.json")
    local assets = HttpService:JSONDecode(gotassets)
    local localassets = readliverfile("assets.json")
    if localassets then
        localassets = HttpService:JSONDecode(localassets)
        if localassets.version ~= assets.version then
            writefile("liver/new/files/assets.json", gotassets)
...
cheat.Library, cheat.Toggles, cheat.Options = loadliverfile("library_main.lua")()
cheat.ThemeManager = loadliverfile("library_theme.lua")()
cheat.SaveManager = loadliverfile("library_save.lua")()
local ui = {
...
do
    ...
    "liver.xyz canary | Jul 28 2024 | fps: 63 ";
    ...
    if canuseimages then rawget(gradr, "_frame").Image = getcustomasset("liver/new/files/grad90r.png"); end
    if canuseimages then rawget(gradl, "_frame").Image = getcustomasset("liver/new/files/grad90l.png"); end
    ...
    cheat.utility.new_renderstepped(LPH_JIT_MAX(function(delta)
        ...
        watertext = ("liver %s | %s | %s | %s fps"):format(SWG_Version, SWG_ShortName, os.date("%b %d %Y"), tostring(finalfps));
...
loadliverfile("chat_spam.lua")(cheat.Library, ui.box.misc:AddTab("chat spam"), function(word)
...
cheat.ThemeManager:SetFolder('liver')
cheat.SaveManager:SetFolder('liver')
